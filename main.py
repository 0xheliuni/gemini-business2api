...
def save_task_to_history(task_type: str, task_data: dict) -> None:
    ...
    with task_history_lock:
        ...
        logger.info(
            f"[HISTORY] Saved {task_type} task to history: {history_entry['id']}"
        )


def _build_history_entry(
    task_type: str, task_data: dict, is_live: bool = False
) -> dict:
    total_value = (
        task_data.get("count")
        if task_type == "register"
        else len(task_data.get("account_ids", []))
    )
    return {
        "id": task_data.get("id", ""),
...
multi_account_mgr = load_multi_account_config(
    http_client, USER_AGENT, RETRY_POLICY, SESSION_CACHE_TTL_SECONDS, global_stats
)
        ...
        try:
            ...
            if _last_known_accounts_version != db_version:
                ...
                for acc_id, acc_mgr in multi_account_mgr.accounts.items():
                    if (
                        not acc_mgr.config.is_expired()
                        and not acc_mgr.config.disabled
                        and not acc_mgr.is_available
                    ):
                        if not acc_mgr.quota_cooldowns:
                            acc_mgr.is_available = True
                            logger.info(
                                f"[AUTO-REFRESH] 账号 {acc_id} 状态已修正为可用"
                            )
...
async def startup_event():
    ...
    for account_id, account_mgr in multi_account_mgr.accounts.items():
        account_mgr.conversation_count = global_stats["account_conversations"].get(
            account_id, 0
        )
    ...
    if expire_hours < 0:
        ...
        logger.info(
            f"[SYSTEM] 媒体文件过期清理任务已启动（过期时间: {expire_hours}小时，检查间隔: 30分钟）"
        )
...
async def shutdown_event():
    ...
    if storage.is_database_enabled():
        try:
            ...
            logger.info(
                f"[SYSTEM] 应用关闭，已保存 {success_count}/{len(multi_account_mgr.accounts)} 个账户的冷却状态"
            )
        except Exception as e:
    ...
    while True:
        try:
            ...
            for attempt in range(3):
                try:
                    success_count = await account.save_all_cooldown_states(
                        multi_account_mgr
                    )
                    logger.debug(
                        f"[COOLDOWN] 定期保存: {success_count}/{len(multi_account_mgr.accounts)} 个账户"
                    )
                    break
                except Exception as retry_err:
                    err_msg = str(retry_err)
                    if (
                        "another operation" in err_msg
                        or "ConnectionDoesNotExist" in err_msg
                        or "connection was closed" in err_msg
                    ):
                        if attempt < 2:
                            logger.warning(
                                f"[COOLDOWN] 数据库连接繁忙，{attempt + 1}/3 次重试..."
                            )
    ...
    for directory, url_prefix, media_type in [
        ...
        for filename in os.listdir(directory):
            ...
            try:
                ...
                mtime = stat.st_mtime
                size = stat.st_size
                created_at = datetime.fromtimestamp(mtime, tz=beijing_tz).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                # 计算剩余有效时间
                if expire_hours > 0:
...
async def admin_cleanup_expired(request: Request):
    ...
    if deleted_count > 0:
        logger.info(
            f"[GALLERY] 手动清理了 {deleted_count} 个过期媒体文件（图片: {deleted_images}, 视频: {deleted_videos}）"
        )
...
async def cleanup_expired_media_task():
    ...
    while True:
        try:
            ...
            if deleted_count > 0:
                logger.info(
                    f"[GALLERY] 清理了 {deleted_count} 个过期媒体文件（过期时间: {expire_hours}小时）"
                )
...
def get_all_model_ids() -> List[str]:
    """统一返回当前项目可用的模型集合。"""
    return list(dict.fromkeys(list(MODEL_MAPPING.keys()) + list(VIRTUAL_MODELS.keys())))


def build_gemini_model_payload(model_name: str) -> Dict[str, Any]:
    """构造最小可用的 Gemini 风格模型元数据。"""
    return {
        "name": f"models/{model_name}",
        "baseModelId": model_name,
        "version": "001",
        "displayName": model_name,
        "description": f"Gemini Business compatible model: {model_name}",
        "inputTokenLimit": 0,
        "outputTokenLimit": 0,
...
def resolve_api_auth_value(
    request: Request, authorization: Optional[str], x_goog_api_key: Optional[str]
) -> Optional[str]:
    """兼容 OpenAI / Gemini 常见鉴权位置。"""
    if authorization:
...
def gemini_request_to_chat_request(
    model_name: str, gemini_req: GeminiGenerateRequest, is_stream: bool
) -> ChatRequest:
    """将 Gemini generateContent 请求适配为现有 ChatRequest。"""
    messages: List[Message] = []

    if gemini_req.systemInstruction and gemini_req.systemInstruction.parts:
        system_text = "".join(
            part.text or "" for part in gemini_req.systemInstruction.parts
        ).strip()
        if system_text:
    ...
    generation_config = gemini_req.generationConfig or {}
    return ChatRequest(
        model=normalize_gemini_model_name(model_name) or "gemini-auto",
        messages=messages,
        stream=is_stream,
        temperature=generation_config.get("temperature", 0.7),
        top_p=generation_config.get("topP", 1.0),
    )


def build_gemini_usage_metadata(usage: Dict[str, Any]) -> Dict[str, int]:
    """将现有 usage 结构映射为 Gemini usageMetadata。"""
    return {
...
def build_gemini_generate_response(
    chat_response: Dict[str, Any], model_name: str
) -> Dict[str, Any]:
    """将现有 OpenAI 风格响应包装为 Gemini generateContent 响应。"""
    ...
    if isinstance(reasoning_content, str) and reasoning_content:
        parts.append({"text": reasoning_content})
    ...
    return {
        ...
        "usageMetadata": build_gemini_usage_metadata(chat_response.get("usage") or {}),
        "modelVersion": normalize_gemini_model_name(model_name),
    }


async def gemini_stream_from_openai_response(
    source_response: StreamingResponse, model_name: str
):
    """将现有 OpenAI SSE 分片转换为 Gemini SSE 分片。"""
    buffer = ""

    async for raw_chunk in source_response.body_iterator:
        chunk_text = (
        ...
        while "\n\n" in buffer:
            ...
            text_value = delta.get("content") or delta.get("reasoning_content")
            ...
            if finish_reason:
                gemini_chunk["candidates"][0]["finishReason"] = str(
                    finish_reason
                ).upper()

            if text_value or finish_reason:
                yield f"data: {json.dumps(gemini_chunk, ensure_ascii=False)}\n\n"


def parse_gemini_action(model_action: str) -> tuple[str, str, bool]:
    """解析 Gemini 模型动作路径。"""
    action_path = model_action.lstrip("/")
    if ":" not in action_path:
        raise HTTPException(status_code=400, detail="Missing Gemini action suffix")

    model_name, action = action_path.rsplit(":", 1)
    if action not in {"generateContent", "streamGenerateContent"}:
        raise HTTPException(
            status_code=404, detail=f"Unsupported Gemini action: {action}"
        )
...
async def admin_stats(request: Request, time_range: str = "24h"):
    ...
    for account_manager in multi_account_mgr.accounts.values():
        ...
        is_rate_limited = (
            cooldown_seconds > 0 and cooldown_reason and "冷却" in cooldown_reason
        )
        is_failed = is_expired
        is_active = (
            (not is_failed) and (not is_manual_disabled) and (not is_rate_limited)
        )
        ...
        accounts_info.append(
            {
                ...
                "disabled_reason": getattr(account_manager, "disabled_reason", None)
                or getattr(config, "disabled_reason", None),
...
async def admin_start_register(
    ...
    task = await register_service.start_register(
        count=count, domain=domain, mail_provider=mail_provider
    )
    return task.to_dict()
...
async def admin_bulk_delete_accounts(request: Request, account_ids: list[str]):
    ...
    if len(account_ids) > 50:
        raise HTTPException(
            400, f"单次最多删除50个账户，当前请求 {len(account_ids)} 个"
        )
    if not account_ids:
    ...
    try:
        ...
        if browser_mode_raw is not None and str(browser_mode_raw).strip():
            ...
            if browser_mode not in ("normal", "silent", "headless"):
                raise HTTPException(
                    status_code=400,
                    detail="browser_mode 必须是 normal / silent / headless",
                )
        else:
            browser_headless = _parse_bool(
                basic.get("browser_headless"), config.basic.browser_headless
            )
        ...
        retry.setdefault(
            ...
            config.retry.text_rate_limit_cooldown_seconds,
        )
        retry.setdefault(
            "images_rate_limit_cooldown_seconds",
            config.retry.images_rate_limit_cooldown_seconds,
        )
        retry.setdefault(
            "videos_rate_limit_cooldown_seconds",
            config.retry.videos_rate_limit_cooldown_seconds,
        )
        retry.setdefault(
            "verification_code_resend_count",
            config.retry.verification_code_resend_count,
        )
        ...
        retry_changed = (
            old_retry_config["text_rate_limit_cooldown_seconds"]
            != RETRY_POLICY.cooldowns.text
            or old_retry_config["images_rate_limit_cooldown_seconds"]
            != RETRY_POLICY.cooldowns.images
            or old_retry_config["videos_rate_limit_cooldown_seconds"]
            != RETRY_POLICY.cooldowns.videos
...
async def admin_get_task_history(request: Request, limit: int = 100):
            ...
            if current_register and current_register.status in ("running", "pending"):
                live_entries.append(
                    _build_history_entry(
                        "register", current_register.to_dict(), is_live=True
                    )
                )
        if login_service:
            ...
            if current_login and current_login.status in ("running", "pending"):
                live_entries.append(
                    _build_history_entry("login", current_login.to_dict(), is_live=True)
                )
    except Exception as exc:
    ...
    # 限制返回数量
    limit = min(limit, 100)
    return {"total": len(history), "limit": limit, "history": history[:limit]}
...
async def list_gemini_models(
    ...
    verify_api_key(
        API_KEY, resolve_api_auth_value(request, authorization, x_goog_api_key)
    )
    return {
        "models": [
            build_gemini_model_payload(model_name) for model_name in get_all_model_ids()
        ]
    }
...
async def get_gemini_model(
    ...
    if normalized_model not in get_all_model_ids():
        raise HTTPException(
            status_code=404, detail=f"Model '{normalized_model}' not found"
        )
    return build_gemini_model_payload(normalized_model)
...
async def gemini_generate_content(
    model_action: str,
    gemini_req: GeminiGenerateRequest,
    request: Request,
    authorization: Optional[str] = Header(None),
    x_goog_api_key: Optional[str] = Header(None),
):
    ...
    model_name, _action, is_stream_action = parse_gemini_action(model_action)
    if model_name not in get_all_model_ids():
        raise HTTPException(status_code=404, detail=f"Model '{model_name}' not found")

    is_stream = is_stream_action or request.query_params.get("alt") == "sse"
    chat_req = gemini_request_to_chat_request(model_name, gemini_req, is_stream)
    chat_response = await chat_impl(chat_req, request, authorization)

    if is_stream:
        return StreamingResponse(
            gemini_stream_from_openai_response(chat_response, model_name),
            media_type="text/event-stream",
        )

    return build_gemini_generate_response(chat_response, model_name)
...
# chat实现函数
async def chat_impl(req: ChatRequest, request: Request, authorization: Optional[str]):
    # 生成请求ID（最优先，用于所有日志追踪）
    ...
    async def finalize_result(
        ...
        async with stats_lock:
            ...
            if status == "success" and latency_ms is not None:
                # 记录首响时间和完成时间，按模型分类
                ttfb_ms = (
                    int((first_response_time - start_ts) * 1000)
                    if first_response_time
                    else latency_ms
                )
    ...
    async with session_lock:
        ...
        if cached_session:
            ...
            try:
                account_manager = await multi_account_mgr.get_account(
                    account_id, request_id, required_quota_types
                )
        ...
        if not cached_session:
            ...
            available_accounts = multi_account_mgr.get_available_accounts(
            ...
            for retry_idx in range(max_retries):
                try:
                    ...
                    logger.error(
                        f"[CHAT] [req_{request_id}] 账户 {account_id} 创建会话失败 (尝试 {retry_idx + 1}/{max_retries}) - {error_type}: {str(e)}"
                    )
                    # 记录账号池状态（单个账户失败）
                    status_code = (
                    ...
                    if retry_idx == max_retries - 1:
                        ...
                        raise HTTPException(
                            503, f"All accounts unavailable: {str(last_error)[:100]}"
                        )
                    # 继续尝试下一个账户
        ...
        for retry_idx in range(max_retries):
            try:
                ...
                if getattr(request.state, "first_response_time", None) is None:
                    # 空响应应该触发重试逻辑
                    raise HTTPException(
                        status_code=502, detail="Empty response from upstream"
                    )
                ...
                if is_http_exception:
                    ...
                    account_manager.handle_non_http_error(
                        "聊天请求", request_id, quota_type
                    )
...
async def generate_images(
    ...
    logger.info(
        f"[IMAGE-GEN] [req_{request_id}] 收到图片生成请求: model={req.model}, prompt={req.prompt[:100]}"
    )
    ...
    try:
        ...
        logger.info(
            f"[IMAGE-GEN] [req_{request_id}] 使用系统配置: {system_format} -> {response_format}"
        )
                ...
                for url in url_matches[: req.n]:
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            data_list.append(
                                {"b64_json": b64_data, "revised_prompt": req.prompt}
                            )
                    except Exception as e:
                        logger.error(
                            f"[IMAGE-GEN] [req_{request_id}] 下载图片失败: {url}, {str(e)}"
                        )
        else:
            # 返回 URL 格式
            for url in url_matches[: req.n]:
            ...
            if not data_list and b64_matches:
                ...
                for idx, (mime, b64_data) in enumerate(b64_matches[: req.n], 1):
                    try:
                        ...
                        url = save_image_to_hf(
                            img_data, chat_id, file_id, mime, base_url, IMAGE_DIR
                        )
                        data_list.append({"url": url, "revised_prompt": req.prompt})
                    except Exception as e:
                        logger.error(
                            f"[IMAGE-GEN] [req_{request_id}] 保存图片失败: {str(e)}"
                        )
        ...
        logger.error(
            f"[IMAGE-GEN] [req_{request_id}] 图片生成失败: {type(e).__name__}: {str(e)}"
        )
        raise
    ...
    try:
        ...
        if mask:
            ...
            content_parts.insert(
                1, {"type": "image_url", "image_url": {"url": mask_uri}}
            )
            logger.info(
                f"[IMAGE-EDIT] [req_{request_id}] 包含遮罩图片: {len(mask_bytes)} bytes"
            )
        ...
        logger.info(
            f"[IMAGE-EDIT] [req_{request_id}] 使用系统配置: {system_format} -> {fmt}"
        )
                ...
                for url in url_matches[:n]:
                    try:
                        ...
                        if resp.status_code == 200:
                            b64_data = base64.b64encode(resp.content).decode()
                            data_list.append(
                                {"b64_json": b64_data, "revised_prompt": prompt}
                            )
                    except Exception as e:
                        logger.error(
                            f"[IMAGE-EDIT] [req_{request_id}] 下载图片失败: {url}, {str(e)}"
                        )
        else:
            for url in url_matches[:n]:
            ...
            if not data_list and b64_matches:
                ...
                for idx, (mime, b64_data) in enumerate(b64_matches[:n], 1):
                    try:
                        ...
                        logger.error(
                            f"[IMAGE-EDIT] [req_{request_id}] 保存图片失败: {str(e)}"
                        )
        ...
        logger.error(
            f"[IMAGE-EDIT] [req_{request_id}] 图片编辑失败: {type(e).__name__}: {str(e)}"
        )
        raise
    ...
    for data in data_list:
        ...
        for reply in replies:
            ...
            if file_info and file_info.get("fileId"):
                ...
                logger.debug(
                    f"[PARSE] 解析文件: fileId={file_id}, mimeType={mime_type}"
                )
                file_ids.append({"fileId": file_id, "mimeType": mime_type})
...
async def stream_chat_generator(
    ...
    async with http_client_chat.stream(
        ...
        try:
            ...
            async for json_obj in parse_json_array_stream_async(r.aiter_lines()):
                ...
                # 记录原始响应结构（用于调试空响应）
                logger.debug(
                    f"[API] [{account_manager.config.account_id}] [req_{request_id}] 收到响应#{response_count}: {json.dumps(json_obj, ensure_ascii=False)[:1000]}"
                )
                ...
                if "error" in json_obj:
                    ...
                    logger.warning(
                        f"[API] [{account_manager.config.account_id}] [req_{request_id}] 上游返回错误: {json.dumps(error_info, ensure_ascii=False)}"
                    )

                    # 上游 429 配额耗尽：立即标记冷却并抛异常，触发切换账户
                    if error_code == 429 or "RESOURCE_EXHAUSTED" in error_info.get(
                        "status", ""
                    ):
                        quota_type = get_request_quota_type(model_name)
                        account_manager.handle_http_error(
                            429, error_message[:200], request_id, quota_type
                        )
                        raise HTTPException(
                            status_code=429,
                            detail=f"Upstream quota exhausted: {error_message[:200]}",
                        )
                ...
                if answer_state == "SKIPPED":
                    ...
                    if "CUSTOMER_POLICY_VIOLATION" in skip_reasons:
                        ...
                        for policy in policy_results:
                            ...
                            if armor_result:
                                violation_detail = armor_result.get(
                                    "modelArmorViolation", ""
                                )
                                if violation_detail:
                                    break
                        ...
                        logger.warning(
                            f"[API] [{account_manager.config.account_id}] [req_{request_id}] 内容被安全策略阻止: {violation_detail or 'CUSTOMER_POLICY_VIOLATION'}"
                        )
                        ...
                        logger.warning(
                            f"[API] [{account_manager.config.account_id}] [req_{request_id}] 响应被跳过: {reason_text}"
                        )
                ...
                if not replies:
                    logger.debug(
                        f"[API] [{account_manager.config.account_id}] [req_{request_id}] 响应#{response_count}无replies，完整answer结构: {json.dumps(answer, ensure_ascii=False)[:500]}"
                    )
                else:
                    logger.debug(
                        f"[API] [{account_manager.config.account_id}] [req_{request_id}] 响应#{response_count}包含{len(replies)}个replies"
                    )
                ...
                for idx, reply in enumerate(replies):
                    ...
                    if not text:
                        # 记录为什么没有text
                        logger.debug(
                            f"[API] [{account_manager.config.account_id}] [req_{request_id}] Reply#{idx}无text，content_obj结构: {json.dumps(content_obj, ensure_ascii=False)[:300]}"
                        )
                        continue
                    ...
                    if not usage_counted:
                        ...
                        account_manager.increment_daily_usage(
                            get_request_quota_type(model_name)
                        )
            ...
            # 记录流处理总结
            logger.info(
                f"[API] [{account_manager.config.account_id}] [req_{request_id}] 流处理完成: 收到{response_count}个响应对象, 累计内容长度{len(full_content)}字符"
            )
            if response_count > 0 and len(full_content) == 0:
                ...
                if quota_type in ("images", "videos"):
                    logger.info(
                        f"[API] [{account_manager.config.account_id}] [req_{request_id}] 媒体生成请求，无文本内容属正常情况"
                    )
                    ...
                    logger.warning(
                        f"[API] [{account_manager.config.account_id}] [req_{request_id}] ⚠️ 空响应警告: 收到{response_count}个响应但无文本内容，可能是思考模型未生成最终回答或上游错误"
                    )
                    # 打印第一个响应对象的完整结构用于调试
                    if json_objects:
                        logger.warning(
                            f"[API] [{account_manager.config.account_id}] [req_{request_id}] 第一个响应完整结构: {json.dumps(json_objects[0], ensure_ascii=False)}"
                        )
                    ...
                    raise HTTPException(
                        status_code=502,
                        detail="Thinking model produced thoughts but no final content",
                    )
    ...
    if full_content:
        ...
        if quota_type in ("images", "videos"):
            logger.info(
                f"[CHAT] [{account_manager.config.account_id}] [req_{request_id}] 媒体生成请求，文本响应为空属正常情况"
            )
        else:
            logger.warning(
                f"[CHAT] [{account_manager.config.account_id}] [req_{request_id}] ⚠️ 最终响应为空，请检查上游日志"
            )
